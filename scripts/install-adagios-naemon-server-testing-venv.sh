...
sudo sed -i '
    ...
    s|/usr/share/okconfig/templates|/etc/naemon/okconfig_templates|g
    s|/etc/nagios/okconfig/examples|/etc/naemon/okconfig_examples|g' /etc/okconfig.conf

sudo mkdir -p /etc/naemon/{okconfig_templates,okconfig_examples}
sudo cp -r /usr/share/okconfig/templates/* /etc/naemon/okconfig_templates/
sudo cp -r /usr/share/okconfig/examples/* /etc/naemon/okconfig_examples/
...
sudo firewall-cmd --reload
sudo yum -y install python-devel python-setuptools python-pip
#sudo useradd adagios
#sudo usermod -a -G adm adagios
#sudo usermod -a -G naemon adagios
#sudo usermod -a -G httpd adagios
#sudo su - adagios
sudo echo "Running Adagios in development mode using Python virtualenv" >> /etc/motd
sudo echo "source venv/bin/activate" >> /etc/motd
sudo echo "cd ~/adagios/adagios" >> /etc/motd
sudo echo "python manage.py runserver 0.0.0.0:8080" >> /etc/motd
sudo echo "tail -f /var/log/naemon/livestatus.log" >> /etc/motd

sed -i '/broker_module/ s/$/ debug=1/' /etc/naemon/module-conf.d/livestatus.cfg
sudo chown -R naemon:naemon /etc/naemon
...
cd ~
export DJANGO_VERSION="Django<1.9"
